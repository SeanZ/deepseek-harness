...
- [x] 提交推送并在无运行会话时部署修复，保留原始日志和回退制品。
...
修复制品提交194d2b5603，部署于20260915-alpha1-prompt-fix，上一release为20260915-alpha1；一致备份20260915-205844-pre-prompt-fix。Linux隔离实例六工具七请求仅1个initial头，296包4447文件核验，保留插件字节一致。切换时76会话、无运行会话，即时校验91个数据文件哈希未变。稍后反馈会话恢复追加1条空载荷session/end-seed，逐字节验证原压缩流、解压全文和旧事件仍为完整前缀。服务active/running且NRestarts=0，530个profile链接均解析到新release，公网5条入口跳转Authelia，旧cookie与Origin边界通过。两个隔离服务已停止。
