...
describe('maintained repository reference policy', () => {
  it('允许代理运行手册记录制品提交，但仍校验组织链接和普通文档', (test) => {
    ...
    expect(findRepositoryReferences('.agentdocs/deployment.md', organizationUrl, new Set())).toEqual([
      { file: '.agentdocs/deployment.md', line: 1, kind: 'organization-url' },
    ])
    expect(findRepositoryReferences('docs/deployment.md', fixture.commit, new Set([fixture.commit]))).toEqual([
      { file: 'docs/deployment.md', line: 1, kind: 'commit-hash' },
    ])
  })
